...
class GameComp extends Component {
    ...
    render() {
        if ((this.state.game.snap.status === 0 && this.state.game.snap.turn) ||
            this.state.game.snap.isOver) { // not moving token and human turn or game is over
            this.saveGameState();
        }
        ...
        let isOver = this.state.game.snap.isOver;
        return (
            <div>
